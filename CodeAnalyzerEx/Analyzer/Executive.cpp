...
unordered_map<std::string, std::vector<std::string>> CodeAnalysis::CodeAnalysisExecutive::buildLazyTable(DependencyTable & obj)
{
	unordered_map<std::string, std::vector<std::string>> dependentFiles;
	vector<string> keys = obj.getDBKeys();
	for (std::string key : keys)
	{
		Element<std::string> element = obj.getRecord(key);
		std::vector<std::string> child = element.children;


		std::stack<std::string> fn = obj.split(obj.split(key, '\\').top(), '.');
		if (fn.top() == "h")
		{
			fn.pop();
			child.push_back(fn.top() + ".cpp");
		}

		std::vector<std::string> newchild;
		for (std::string k : child)
		{
			newchild.push_back(FileSystem::Path::getName(k) + ".htm");
			//newchild.push_back(dp.split(k, '\\').top() + ".htm");
		}
		dependentFiles[obj.split(key, '\\').top() + ".htm"] = newchild;
	}
...
std::unordered_map<std::string, std::vector<std::string>> CodeAnalysis::CodeAnalysisExecutive::startCodeAnalaysis(std::string category)
{
	std::cout << "*************REQUIREMENT 4 : Invoking Repository Program satisfy the requirements of CodePublisher developed in Project #3.************\n";
	getSourceFiles();
	...
	DependencyTable dp(getDepXMLName());
	std::cout << "\n*************************************Building Dependency Table******************************************\n";
	std::cout << "\n*************************************Processing******************************************\n";
	dp.buildDependencyTable(getFiles(), tb);

	unordered_map<std::string,std::vector<std::string>> table= buildLazyTable(dp);
	/*std::stack<std::string> fn = split(split(filename, '\\').top(), '.');
	std::vector<std::string> children = record.children;
	depTags = depTags + "<ul>\n";
	if (fn.top() == "h")*/
	//CodePublisher object
	CodePublisher publisher(dp, tb.getButtonMap(),category);
	publisher.publishFiles(getFiles());
	return table;
